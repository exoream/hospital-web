...
const Header = () => {
    ...
    const fetchNotifikasi = async () => {
        try {
            ...
            const res = await fetch('http://localhost:3000/api/admin/notifikasi', {
    ...
    const markAllNotifikasiAsRead = async (fetchNotifikasi) => {
        try {
            const token = await getToken();
            await fetch('http://localhost:3000/api/admin/notifikasi/baca', {
                method: 'PATCH',
                headers: {
                    ...
                    <div className="relative" ref={notifRef}>
                        ...
                        {notifOpen && (
                            <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded-md shadow-lg z-20">
                                <div className="px-4 py-2 font-semibold text-sm border-b">Notifikasi</div>
                                <div className="max-h-64 overflow-y-auto">

                                    {(() => {
                                        if (!notifikasi || notifikasi.length === 0) {
                                            return <div className="p-4 text-sm text-gray-500">Tidak ada notifikasi.</div>;
                                        }

                                        return notifikasi.map((item) => (
                                            <div
                                                key={`${item.tipe}-${item.sumber_id}`}
                                                className={`px-4 py-3 text-sm hover:bg-gray-50 cursor-pointer border-l-2 ${item.status === 'canceled'
                                            ...
                                            >
                                                <div className="flex items-start gap-3">
                                                    <div
                                                        className={`w-8 h-8 rounded-full flex items-center justify-center ${item.status === 'canceled'
                                                            ? 'bg-red-100'
                                                            : item.tipe === 'riwayat'
                                                                ? 'bg-blue-100'
                                                                : 'bg-green-100'
                                                    ...
                                                    >
                                                        <svg
                                                            className={`w-4 h-4 ${item.status === 'canceled' ? 'text-red-600' :
                                                                item.tipe === 'riwayat' ? 'text-blue-600' : 'text-green-600'
                                                                }`}
                                                            fill="none"
                                                            stroke="currentColor"
                                                            viewBox="0 0 24 24"
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        {item.tipe === 'jadwal' && (
                                                            <div className="text-gray-500 text-xs mt-1 flex items-center gap-2">
                                                                <span>
                                                                    {item.tanggal_kunjungan} pukul {item.waktu_kunjungan}
                                                                </span>

                                                                {item.status && (
                                                                    <span
                                                                        className={`px-2 py-0.5 rounded-full text-xs font-semibold text-white ${item.status === 'done'
                                                                            ? 'bg-green-500'
                                                                            : item.status === 'canceled'
                                                                                ? 'bg-red-500'
...
const Content = ({ activeMenu, setActiveMenu }) => {
    ...
    const fetchNotifikasi = async (page, limit, tanggalFilter) => {
        try {
            ...
            console.log(`Fetching notifikasi with query: http://localhost:3000/api/admin/all-notifikasi?${query.toString()}`);
            const res = await fetch(`http://localhost:3000/api/admin/all-notifikasi?${query.toString()}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const resChart = await fetch('http://localhost:3000/api/admin/chart-kunjungan', {
                    headers: { Authorization: `Bearer ${token}` },
                });
        ...
        return (
            <div className="p-6 space-y-6">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-500">Selamat datang di Sistem Informasi Puskesmas Camba</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold text-gray-700">Statistik Kunjungan</h3>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold text-gray-700">Notifikasi Terbaru</h3>
                        <div className="mb-4">
                            <input
                                type="date"
                            ...
                            {notifikasi.map((item) => (
                                ...
                                >
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0
                                        ${item.status === 'canceled' ? 'bg-red-100' :
                                            item.status === 'done' ? 'bg-green-100' :
                                                item.status === 'rescheduled' ? 'bg-yellow-100' : 'bg-blue-100'}`}>
                                        <svg
                                            className={`w-5 h-5 ${item.status === 'canceled' ? 'text-red-600' :
                                                item.status === 'done' ? 'text-green-600' :
                                                    item.status === 'rescheduled' ? 'text-yellow-600' :
                                                        'text-blue-600'
                                                }`}
                                            fill="none"
                                            stroke="currentColor"
    ...
    const renderComponent = () => {
        ...
        switch (activeMenu) {
            ...
            case 'Daftar Pasien > Data Pasien':
                return <DataPatient onSelectDetail={(pasien) => {
                    setSelectedPasien(pasien);
                    setActiveMenu('Riwayat Pasien');
                }} activeMenu={activeMenu} />;

            case 'Daftar Konsultasi Pasien':
                return <PasienHealthRecords onSelectDetail={(pasien) => {
                    setSelectedPasien(pasien);
                ...
                return (
                    <div className="p-6">
                        <div className="mb-6">
                            ...
                            <p className="text-gray-500 text-sm">Menampilkan konten untuk {activeMenu}</p>
                        </div>

                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            <div className="flex items-center justify-center h-40 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                <div className="text-center">
                                    <div className="text-gray-400 mb-2">Konten untuk {activeMenu}</div>
                                    <div className="text-sm text-gray-500">Halaman ini sedang dalam pengembangan</div>
                                </div>
                            </div>
                        </div>
                    </div>
