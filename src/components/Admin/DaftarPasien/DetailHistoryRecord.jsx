...
export default function PasienHistoryRecord({ id }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const resPasien = await fetch(`http://localhost:3000/api/admin/pasien/${id}`, {
                    headers: {
    ...
    const fetchRiwayat = async () => {
        try {
            const token = getToken();
            const resRiwayat = await fetch(`http://localhost:3000/api/admin/riwayat-pasien/${id}?page=${page}&limit=${limit}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleCreateCatatan = async () => {
        if (!catatan.trim()) {
            setAlert({ show: true, type: 'error', message: 'Catatan tidak boleh kosong' });
            return;
        }
        ...
        try {
            const token = getToken();
            const response = await fetch(`http://127.0.0.1:3000/api/admin/notify-patient/${selectedRiwayat.riwayat_id}`, {
                method: 'POST',
                headers: {
            ...
            setAlert({ show: true, type: 'error', message: 'Terjadi kesalahan saat membuat catatan' });
        } finally {
    ...
    if (loading) {
        return <div className="p-6 text-gray-500 text-sm">Memuat data pasien...</div>;
    }

    if (!pasien) {
        return <div className="p-6 text-gray-500 text-sm">Data pasien tidak ditemukan.</div>;
    }
    ...
    return (
        <>
            ...
            ${alert.type === 'success' ? 'border-green-500' : 'border-red-500'} border-t-4`}>
                            <p className={`text-sm ${alert.type === 'success' ? 'text-green-700' : 'text-red-700'}`}>
                                {alert.message}
                            </p>
                            <button
            ...
            <div className="p-6 bg-white min-h-screen">
                <div className="mb-8 max-w-4xl mx-auto">
                    <h2 className="text-2xl font-bold text-green-800 mb-4 border-b-2 border-green-200 pb-2">Detail Pasien</h2>
                        ...
                        <div className="flex flex-col md:flex-row">
                            ...
                            <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                                    <div className="text-xs text-green-600 uppercase font-semibold mb-1">Nama Lengkap</div>
                                    <div className="text-gray-800 font-medium">{pasien[0].nama_lengkap}</div>
                                </div>

                                <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                                    <div className="text-xs text-green-600 uppercase font-semibold mb-1">NIK</div>
                                    <div className="text-gray-800 font-medium">{pasien[0].nik}</div>
                                </div>

                                <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                                    <div className="text-xs text-green-600 uppercase font-semibold mb-1">Jenis Kelamin</div>
                                ...
                                <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                                    <div className="text-xs text-green-600 uppercase font-semibold mb-1">Tanggal Lahir</div>
                                    <div className="text-gray-800 font-medium flex items-center">
                                        ...
                                        {new Date(pasien[0].tanggal_lahir).toLocaleDateString('id-ID', {
                                            day: 'numeric',
                                            month: 'long',
                                            year: 'numeric'
                                        })}
                                ...
                                <div className="bg-green-50 p-3 rounded-lg border border-green-100 md:col-span-2">
                                    <div className="text-xs text-green-600 uppercase font-semibold mb-1">Alamat</div>
                                    <div className="text-gray-800 font-medium flex">
                                        <svg className="w-4 h-4 mr-1 text-green-600 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <table className="min-w-full divide-y divide-green-200">
                                <thead className="bg-green-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-green-700 uppercase tracking-wider">Tanggal</th>
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-green-700 uppercase tracking-wider">Catatan</th>
                                        <th className="px-4 py-3 text-left text-xs font-semibold text-green-700 uppercase tracking-wider">Ringakasan Kunjungan</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-green-100">
                                    {riwayat.length > 0 ? (
                                        riwayat.map((r, idx) => (
                                            <tr key={r.riwayat_id} className="hover:bg-green-50 transition-colors">
                                                <td className="px-4 py-3 text-sm text-gray-700">{(page - 1) * limit + idx + 1}</td>
                                                <td className="px-4 py-3 text-sm font-medium text-gray-800">{r.alamat}</td>
                                                <td className="px-4 py-3 text-sm text-gray-700">{r.keluhan}</td>
                                                <td className="px-4 py-3 text-sm text-gray-700">
                                                    {new Date(r.riwayat_created_at).toLocaleDateString('id-ID', {
                                                        day: 'numeric',
                                                        month: 'long',
                                                        year: 'numeric'
                                                    })}
                                                ...
                                                <td className="px-4 py-3 text-sm text-gray-700">
                                                    {r.catatan ? (
                                                        <>
                                                            <div className="text-gray-800">{r.catatan}</div>
                                                            <div className="text-xs text-gray-400 mt-1">
                                                                Catatan ditulis pada tanggal {new Date(r.catatan_created_at).toLocaleDateString('id-ID')}
                                                            </div>
                                                        </>
                                                    ) : (
                                                        <span className="italic text-gray-400">Belum ada catatan</span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-3 text-sm text-gray-700">
                                                    {r.jadwal_keterangan ? (
                                                        <>
                                                            <div className="text-gray-800">{r.jadwal_keterangan}</div>
                                                            <div className="text-xs text-gray-400 mt-1">
                                                                Kunjungan selesai pada {new Date(r.jadwal_updated_at).toLocaleDateString('id-ID')}
                                                            </div>
                                                        </>
                                                    ) : (
                                                        <span className="italic text-gray-400">Tidak ada ringkasan hasil kunjungan</span>
                                                    )}
                                                </td>
                                            </tr>
                                        ))
                                        ...
                                        <tr>
                                            <td colSpan="7" className="text-center py-8 text-gray-500">
                                                ...
                                                <span className="block text-sm">Belum ada riwayat kesehatan.</span>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 px-4">
                        <div className="bg-white p-6 rounded-xl shadow-xl max-w-md w-full border-2 border-green-200">
                            ...
                            <div className="space-y-3 mb-4 bg-green-50 p-3 rounded-lg border border-green-100">
                                <div className="flex">
                                    <span className="text-xs text-green-600 uppercase font-semibold w-24">Penyakit:</span>
                                    <span className="text-gray-800 font-medium">{selectedRiwayat.penyakit}</span>
                                </div>
                                <div className="flex">
                                    <span className="text-xs text-green-600 uppercase font-semibold w-24">Keluhan:</span>
                                    <span className="text-gray-800">{selectedRiwayat.keluhan}</span>
                                </div>
                                <div className="flex">
                                    <span className="text-xs text-green-600 uppercase font-semibold w-24">Tanggal:</span>
                                    <span className="text-gray-800">{new Date(selectedRiwayat.created_at).toLocaleDateString('id-ID', {
                                        day: 'numeric',
                                        month: 'long',
                                        year: 'numeric'
                                    })}</span>
                            ...
                            <label className="block text-sm font-medium text-green-700 mb-2">Catatan Medis:</label>
                            <textarea
                                value={catatan}
                                onChange={handleCatatanChange}
                                className={`w-full p-3 border-2 ${selectedRiwayat.catatan ? 'border-green-200 bg-green-50' : 'border-green-300 bg-white focus:ring-2 focus:ring-green-200 focus:border-green-400'} rounded-lg text-gray-700 transition-colors ${selectedRiwayat.catatan ? '' : 'focus:outline-none'}`}
                                rows={4}
                                disabled={!!selectedRiwayat.catatan}
                                placeholder={selectedRiwayat.catatan ? "" : "Masukkan catatan medis di sini..."}
                            />
