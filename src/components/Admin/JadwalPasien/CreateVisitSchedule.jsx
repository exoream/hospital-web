...
export default function CreateVisitSchedule() {
  ...
  const keluhanList = pasienList.filter(
    (p) => String(p.pasien_id) === String(formData.pasien_id)
  );
    ...
    const fetchPasien = async () => {
      try {
        ...
        const response = await fetch(
          "https://hospital-be-chi.vercel.app/api/admin/pasien-konsultasi",
          {
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://hospital-be-chi.vercel.app/api/admin/jadwal-kunjungan",
        {
          method: "POST",
          headers: {
              ...
              >
                ...
                {keluhanList.map((item) => (
                  <option key={item.riwayat_id} value={item.riwayat_id}>
                    {item.keluhan} —{" "}
                    {new Date(item.riwayat_created_at).toLocaleDateString()}
                  </option>
                ))}
              </select>
            </div>
