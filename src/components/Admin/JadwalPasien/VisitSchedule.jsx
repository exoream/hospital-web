...
export default function JadwalKunjungan({ activeMenu }) {
  ...
  const fetchJadwal = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://hospital-be-chi.vercel.app/api/admin/jadwal-kunjungan?page=${page}&limit=${limit}`,
        {
          headers: {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(
        `https://hospital-be-chi.vercel.app/api/admin/jadwal-kunjungan/${formData.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const confirmAction = async () => {
    if (modal.action === "save") {
      ...
      await handleSaveWithStatus(
        "done",
        confirmKeterangan.keterangan,
        confirmKeterangan.id
      );
    }
    closeModal();
  };
  ...
  const handleSaveWithStatus = async (status, keterangan, id) => {
    try {
      ...
      const response = await fetch(
        `https://hospital-be-chi.vercel.app/api/admin/jadwal-kunjungan/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...
            {loading ? (
              ...
              <tr>
                <td
                  ...
                  className="text-center py-6 text-gray-500 text-sm"
                >
                  Tidak ada data jadwal kunjungan.
                </td>
              </tr>
            ) : (
