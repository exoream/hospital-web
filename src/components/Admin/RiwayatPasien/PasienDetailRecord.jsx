...
export default function PasienDetail({ id }) {
    ...
    const fetchRiwayat = async () => {
        try {
            const token = getToken();
            const resRiwayat = await fetch(`http://localhost:3000/api/admin/konsultasi-pasien/${id}?page=${page}&limit=${limit}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
                                            ...
                                            <tr key={r.riwayat_id} className="hover:bg-green-50 transition-colors">
                                                ...
                                                <td className="px-4 py-3">
                                                    <button
                                                        ...
                                                        className={`px-3 py-1 text-sm ${r.catatan ? 'bg-green-600 hover:bg-green-700' : 'bg-yellow-500 hover:bg-yellow-600'} text-white rounded-lg shadow-sm transition-colors focus:ring-2 focus:ring-offset-2 ${r.catatan ? 'focus:ring-green-500' : 'focus:ring-yellow-400'}`}
                                                    >
