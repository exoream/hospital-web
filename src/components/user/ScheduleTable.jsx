...
const TabelJadwalKunjungan = () => {
    ...
    const fetchJadwal = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `http://localhost:3000/api/pasien/jadwal-kunjungan?page=${page}&limit=${limit}`,
                {
                    headers: {
    ...
    const handleStatusUpdate = async (id, newStatus) => {
        try {
            const token = getToken();
            const response = await fetch(
                `http://localhost:3000/api/pasien/jadwal-kunjungan/${id}`,
                {
                    method: "PUT",
                    headers: {
    ...
    const StatusBadge = ({ status }) => {
        ...
        return (
            <span className={`px-3 py-1 ${bgColor} ${textColor} rounded-full text-xs font-medium`}>
                {statusText}
            </span>
        );
    };
        ...
        <div className="p-6 bg-green-50 rounded-xl">
            <div className="bg-white p-6 rounded-lg shadow-md border border-green-100">
                <div className="flex items-center mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-green-800">Daftar Jadwal Kunjungan</h2>
                </div>
                        ...
                        <table className="min-w-full bg-white rounded-lg overflow-hidden">
                            <thead>
                                <tr className="bg-green-600 text-white text-left">
                                    <th className="px-6 py-3 text-sm font-semibold uppercase tracking-wider w-12">#</th>
                                    <th className="px-6 py-3 text-sm font-semibold uppercase tracking-wider">Tanggal</th>
                                    <th className="px-6 py-3 text-sm font-semibold uppercase tracking-wider w-24">Waktu</th>
                                    <th className="px-6 py-3 text-sm font-semibold uppercase tracking-wider">Keluhan</th>
                                    <th className="px-6 py-3 text-sm font-semibold uppercase tracking-wider text-center">Status</th>
                                    <th className="px-6 py-3 text-sm font-semibold uppercase tracking-wider text-center">Aksi</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-6 py-10 text-center">
                                            <div className="flex flex-col items-center justify-center text-gray-500">
                                                <svg className="w-12 h-12 text-green-200 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                                </svg>
                                                <p className="font-medium">Tidak ada jadwal kunjungan tersedia</p>
                                                <p className="text-sm mt-1">Jadwal kunjungan yang telah dibuat akan muncul di sini</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                        ...
                        <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                            ...
                            <div className="flex justify-end gap-3">
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-md text-sm text-white ${modal.action === "confirmed"
